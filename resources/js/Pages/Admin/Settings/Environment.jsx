...
export default function Environment({ env = {}, systemInfo = {} }) {
    ...
    const [testEmailAddress, setTestEmailAddress] = useState(systemInfo.admin_email || '');
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        MAIL_PASSWORD: '', // Keep empty by default so secrets are never echoed
        ...
        DB_PASSWORD: '', // Keep empty by default so secrets are never echoed
        ...
        AWS_SECRET_ACCESS_KEY: '', // Keep empty by default so secrets are never echoed
        ...
        REDIS_PASSWORD: '', // Keep empty by default so secrets are never echoed
    });
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        placeholder={env.HAS_MAIL_PASSWORD ? '•••••••• (unchanged)' : 'Enter mail password'}
                                ...
                                <p className="mt-1 text-[11px] text-slate-400">
                                    {env.HAS_MAIL_PASSWORD ? 'Password is saved. Leave blank to keep unchanged.' : 'No password currently configured.'}
                                </p>
                            </div>
                        ...
                        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40">
                            ...
                            <div className="flex flex-col gap-1 w-full sm:w-auto">
                                ...
                                <div className="text-[10px] text-slate-500 dark:text-slate-400">
                                    Anti-relay policy: Test emails are restricted to admin or sender addresses.
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-6 sm:p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-6">
                        ...
                        {env.IS_DB_LOCKED ? (
                            <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-800 flex items-start gap-3">
                                ...
                                <div className="text-xs text-amber-800 dark:text-amber-200">
                                    <strong className="font-bold">Production Security Guard Active:</strong> Database connection settings are locked in production to prevent unintended disconnection or application downtime. To modify database credentials, update the server's <code className="font-mono bg-amber-200/50 dark:bg-amber-900/50 px-1 py-0.5 rounded">.env</code> directly.
                                </div>
                            </div>
                        ) : (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                            <div>
                                ...
                                <select
                                    ...
                                    disabled={Boolean(env.IS_DB_LOCKED)}
                                    onChange={(e) => setData('DB_CONNECTION', e.target.value)}
                                    className={`w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm font-semibold focus:ring-2 focus:ring-blue-500 focus:outline-hidden ${env.IS_DB_LOCKED ? 'opacity-60 cursor-not-allowed bg-slate-100 dark:bg-slate-900' : ''}`}
                                >
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className={`w-full pl-4 pr-10 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm font-semibold focus:ring-2 focus:ring-blue-500 focus:outline-hidden ${env.IS_DB_LOCKED ? 'opacity-60 cursor-not-allowed bg-slate-100 dark:bg-slate-900' : ''}`}
                                        placeholder={env.HAS_DB_PASSWORD ? '•••••••• (unchanged)' : 'Enter database password'}
                                        autoComplete="new-password"
                                    />
                                    <button
                                        type="button"
                                ...
                                <p className="mt-1 text-[11px] text-slate-400">
                                    {env.HAS_DB_PASSWORD ? 'Password is saved. Leave blank to keep unchanged.' : 'No database password configured.'}
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full pl-4 pr-10 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm font-semibold focus:ring-2 focus:ring-blue-500 focus:outline-hidden"
                                        placeholder={env.HAS_AWS_SECRET ? '•••••••• (unchanged)' : 'Enter AWS secret key'}
                                        autoComplete="new-password"
                                    />
                                    <button
                                        type="button"
                                ...
                                <p className="mt-1 text-[11px] text-slate-400">
                                    {env.HAS_AWS_SECRET ? 'Secret is saved. Leave blank to keep unchanged.' : 'No secret key configured.'}
                                </p>
                            </div>
